...
export class CLI<R extends Resource, A extends Assembler<R>> {
      ...
      rows.get(path) ??
        (rows.set(path, {
          ...
          impact: {
            ...
            directives: 0,
            isRoutable: false,
            isAutoGenerated: path.indexOf(".auto.") > 0,
          },
  ...
  async ls(opts: {
    ...
    if (opts?.auto) {
      list = list.filter((r) => r.impact.isAutoGenerated);
    }
