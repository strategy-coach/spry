...
export class Foundries {
  ...
  env(step: Workflow["workflowStep"], ce: Foundries["ceSelected"][number]) {
    let ceEnv: Record<string, string> = {
      ...
      FOUNDRY_AUTO_MATERIALIZE: ce.pfn.materialize.auto ? "TRUE" : "FALSE",
  ...
  async materialize(step: Workflow["workflowStep"]) {
          ...
          if (pfn.materialize.auto) {
            try {
              ...
              this.lintr.add({
                ...
                message: `Capturable executable materialization failed: ${
                  JSON.stringify(pfn)
                }`,
    ...
    switch (step) {
      case "BEFORE_ANN_CATALOG":
        for await (
          const ce of this.ceSelected.filter((ce) => ce.ann.runBeforeAnnCatalog)
        ) {
          await execute(ce);
        }
        break;
      case "AFTER_ANN_CATALOG":
        for await (
          const ce of this.ceSelected.filter((ce) => ce.ann.runAfterAnnCatalog)
        ) {
          await execute(ce);
        }
        break;
