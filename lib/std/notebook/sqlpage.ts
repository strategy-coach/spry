...
import * as c from "../../universal/reflect/callable.ts";
 ...
 * This notebook creates `sqlpage_files` table as part of init.
 */
...
export type PathShellConfig = {
  ...
  readonly shellStmts:
    | "do-not-include"
    | ((spfr: SqlPagesFileRecord) => string | string[] | SQL);
...
/**
 * Represents the base class for all SQLPage notebooks (which generate SQLPage content).
 * This class is used to populate SQLPage content for Web UIs in Spry databases eliminating
...
export class TypicalSqlPageNotebook {
  ...
  absoluteURL(relativeURL: string) {
    return SQL`sqlpage.environment_variable('SQLPAGE_SITE_PREFIX') || ${relativeURL}`;
  }

  constructHomePath(parentPath: string) {
    return SQL`${parentPath} || '/index.sql'`;
  }
    ...
    return {
      init: () => {
        const countSQL = config.countSQL
          ? config.countSQL
          : SQL`SELECT COUNT(*) FROM ${config.tableOrViewName} ${
  ...
  breadcrumbsSQL(
    ...
    return SQL`
        ...
        SELECT title, ${this.absoluteURL("/")} || link as link
        FROM breadcrumbs ORDER BY level DESC;${(additional.length
        ? (additional.map((crumb) => raw`\nSELECT ${crumb.title ? literal(crumb.title) : crumb.titleExpr} AS title, '${crumb.link ?? "#"}' AS link;`))
        : raw``)}`;
  }
  ...
  activePageTitle() {
    ...
    return SQL`
          SELECT 'title' AS component, (SELECT COALESCE(title, caption)
              FROM sqlpage_aide_navigation
             WHERE namespace = 'prime' AND path = ${activePPC?.absPath ?? "/"}) as contents;
    `;
  }
...
`;
  ...
  async textFrom(value: unknown, unknownNature: (value: unknown) => string) {
    if (typeof value === "string") {
      ...
      return ensureTrailingSemicolon(value.text());
    } else {
   ...
   * console.log(result); // Outputs the processed string or SQL statements
   */
  async methodText(c: c.Callable<TypicalSqlPageNotebook, Any>) {
    return this.textFrom(
      await c.call(),
      (value) =>
        `\n/* '${String(c.callable)}' in '${
          String(c.source)
        }' returned type ${typeof value} (${
          JSON.stringify(value)
        }) instead of string | string[] | SQLa.SqlTextSupplier */`,
    );
  }
