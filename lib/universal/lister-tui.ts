...
/* ------------------------------------------------------------------------------------------------
 * SmartListerBuilder (fully dynamic, type-only inference from first row)
 * ----------------------------------------------------------------------------------------------*/

/**
 * SmartListerBuilder
 *
 * - No hardcoded names. Keys are taken from the first row (own enumerable props).
 * - Column headers are Title Case of the key string.
 * - Column kind inferred purely by the runtime type of the first row’s value.
 * - Returns a normal ListerBuilder you can keep refining (select/reorder/sort/colors).
 */
export class SmartListerBuilder<T extends Record<string, unknown>> {
    private rows?: Iterish<T>;
    private constructor() {}

    static fromRows<U extends Record<string, unknown>>(rows: Iterish<U>) {
        const s = new SmartListerBuilder<U>();
        s.rows = rows;
        return s;
    }
    ...
    async build() {
        const b = await this.toBuilder();
        return b.build();
    }

    async toBuilder(): Promise<
        ListerBuilder<T, Record<string, unknown>, string>
    > {
        if (!this.rows) throw new Error("[smart] .from(rows) is required.");

        const { first, rebuilt } = await peekIterish(this.rows);
        this.rows = rebuilt;

        if (!first) {
            const empty = new ListerBuilder<
                T,
        ...
        let b = new ListerBuilder<T, Record<string, unknown>, string>()
            .declareColumns(...keys)
            .from(this.rows)
            .header(true)
            .compact(false)
        ...
        let firstStringKey: (keyof T & string) | undefined;

        for (const k of keys) {
            const v = first[k];
            const header = toTitleCase(k);

            if (v instanceof Date) {
                b = b.date(k, (row) => row[k] as Date | string | number, {
                    header,
                });
                continue;
            }
            ...
            switch (typeof v) {
                case "number":
                    b = b.numeric(k, (row) => Number(row[k] as number), {
                        header,
                ...
                case "boolean":
                    // use generic column<boolean> to keep formatter types strict
                    b = b.column<boolean>(k, (row) => Boolean(row[k]), {
                        header,
                ...
                case "string":
                    if (!firstStringKey) firstStringKey = k;
                    b = b.column<string>(k, (row) => String(row[k] ?? ""), {
                        header,
                    });
                    break;

                default:
                    // object / bigint / symbol / undefined / function → stringify safely
                    b = b.column<string>(k, (row) => {
                        const val = row[k];
        ...
        // Friendly default sort: first string key if present, else first key.
        const sortId = (firstStringKey ?? keys[0]) as keyof Record<
            string,
            unknown
        >;
        if (sortId) {
            (b as unknown as ListerBuilder<T, Record<string, unknown>, string>)
                .sortBy(sortId);
            b = b.sortDir("asc");
        }
...
async function peekIterish<U>(
    src: Iterish<U>,
): Promise<{ first?: U; rebuilt: Iterish<U> }> {
    // Synchronous
    if (
        (src as Iterable<U>)[Symbol.iterator] &&
        !(src as AsyncIterable<U>)[Symbol.asyncIterator]
    ) {
        const it = (src as Iterable<U>)[Symbol.iterator]();
        const n = it.next();
        if (n.done) return { first: undefined, rebuilt: [] as U[] };
        // deno-lint-ignore no-inner-declarations
        function* rebuilt(first: U, rest: Iterator<U>) {
            yield first;
            for (let r = rest.next(); !r.done; r = rest.next()) yield r.value;
        }
        return { first: n.value, rebuilt: rebuilt(n.value, it) };
    }
    // Asynchronous
    const ait = (src as AsyncIterable<U>)[Symbol.asyncIterator]();
    const n = await ait.next();
    if (n.done) return { first: undefined, rebuilt: (async function* () {})() };
    async function* rebuiltAsync(head: U, rest: AsyncIterator<U>) {
        yield head;
        for (;;) {
    ...
    return { first: n.value, rebuilt: rebuiltAsync(n.value, ait) };
}
